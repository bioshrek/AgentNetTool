...
const ReviewPage = () => {
  ...
  const handleRegenerateClip = async (stepIndex: number) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5328/api/recording/${recordingName}/regenerate_clip/${stepIndex}/1`,
        { method: "POST" },
      );
      if (!response.ok) {
                          ...
                          <Grid container spacing={2}>
                            ...
                            <Grid>
                              <Tooltip title="Task with a clear and important goal and around 20 steps">
                                <Button
                                  variant="outlined"
                                  onClick={() => setAcceptLevel("excellent")}
                                  sx={{
                                    backgroundColor:
                                      acceptLevel === "excellent"
                                        ? "lightblue"
                                        : "transparent",
                                    borderColor:
                                      acceptLevel === "excellent"
