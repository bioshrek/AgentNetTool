...
const Page = () => {
  ...
  useEffect(() => {
    ...
    setRecordingStatus(recordingData.task_data.recording_status as statusProp);
    setActiveStep(0);
    setCutTaskName("");
    setCutDescription("");
  }, [recordingData]);
  ...
  useEffect(() => {
    if (stepperRef.current) {
      ...
      const stepHeight = stepperRef.current.scrollHeight / eventsList.length;
      const offset =
  ...
  const handleDeleteEvent = async (
    ...
    // revert changes if persistence fails
  ...
  const handleExportRecording = async () => {
    try {
      ...
      if (path) {
        setIsExporting(true);
        const response = await fetch(
          `http://localhost:5328/api/recording/${recordingName}/export`,
          {
            method: "POST",
  ...
  const handleUploadRecording = async () => {
    const saved = await handleConfirmRecording(eventsList, { silent: true });
    if (!saved) {
      return;
    }
    ...
    SocketService.Post("upload_recording", {
      ...
      .then((data) => {
        console.log(`Uploading ${data.recording_name} successfully`);
        showSuccess(`Uploading ${data.recording_name} successfully`);
        clearInterval(interval);
      ...
      .catch((error) => {
        console.error(`Failed in uploading ${error.recording_name} `);
            ...
            >
              <Card>
                <Typography id="interval-slider" gutterBottom>
                  Select the event interval corresponding to the task
                </Typography>
                <Box>
                  {/* Slider */}
                  <Slider
                  ...
                  <Stack direction="row" alignItems="center" spacing={2}>
                    <Typography level="body-md">First</Typography>
                    <div className="mt-2">
                    ...
                    <Typography level="body-md">Last</Typography>
                    <div className="mt-2">
                      <div className="flex rounded-md shadow-sm ring-1 ring-inset  focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                        <input
                          id="valmax"
                    ...
                    <div className="mt-2">
                      <div className="flex rounded-md shadow-sm ring-1 ring-inset  focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                        <input
                          ...
                          value={cutTaskName}
                          onChange={(e) => setCutTaskName(e.target.value)}
                          className="block flex-1 border-0 bg-transparent py-1.5 pl-1 placeholder: focus:ring-0 sm:text-sm sm:leading-6"
                        />
                      </div>
                    </div>
          ...
          <div className="flex items-center col-span-3">
            ...
            >
              ...
              >
                ...
                >
                  <TabPanel value={0} sx={{ mx: -2, bgcolor: "transparent" }}>
                    {videoClipSrc ? (
                      <AspectRatio
                        objectFit="contain"
                        sx={{ height: "100%" }}
                  ...
                  <TabPanel value={1}>
                    {eventsList[activeStep] ? (
                      eventsList[activeStep].axtree ? (
                        ...
                        >
                          <ReactJson src={eventsList[activeStep].axtree} />
                        </div>
                      ) : (
                        <div
                          className="h-full overflow-y-auto"
                        ...
                        >
                          {index === activeStep ? (
                            <EditableText
                              ...
                              onSave={(e) => handleChange(e, index, "action")}
                              row_character_number={18}
                ...
                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div className="sm:flex sm:items-start">
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500">
                          You have unsaved changes made to events, do you want
                          to save them?
                        </p>
                      </div>
                    </div>
